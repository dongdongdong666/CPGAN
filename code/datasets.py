...
def prepare_data(data):
    ...
    captions, captions_lens, class_ids, keys, memory = data
    ...
    # return [real_imgs, captions, sorted_cap_lens, class_ids, keys, memory]
    return [captions, sorted_cap_lens, class_ids, keys, memory]
...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='train',
        ...
        print('Load memory feats from ../memory/memoryft_9487.pkl')
        #
    ...
    def __getitem__(self, index):
        ...
        for i in range(caps.shape[0]):
            cap_id = self.vob_trans[caps[i][0]]
            memory_list.append(self.memory_feats[str(cap_id)])
        memory = np.array(memory_list)
        #

        # return imgs, caps, cap_len, cls_id, key, memory
        return caps, cap_len, cls_id, key, memory
