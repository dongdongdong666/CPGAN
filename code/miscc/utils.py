...
def build_super_images(real_imgs, captions, ixtoword,
                       attn_maps, att_sze, boxes, keys, image_dir, lr_imgs=None,
    ...
    for i in range(num):
        attn = attn_maps[i].cpu().view(1, -1, 10*10)
        attn = attn[:, :, 64:]
        ...
        for j in range(len(sent)):
            temp = deepcopy(img)
            temp_img = Image.fromarray(temp.astype('uint8')).convert('RGB')
            draw = ImageDraw.Draw(temp_img)
            word = sent[j]
            att = attn[j]
            row_index, col_index = np.where(att==np.max(att))
            row_index = row_index[0]
            col_index = col_index[0]
            bbox = box[:, row_index, col_index]
            for n in range(4):
                if bbox[n] > 255:
                    bbox[n] = 254.9
                if bbox[n] < 0:
            ...
            # right_down_point = (bbox[2], bbox[3])
            draw.rectangle((bbox[0], bbox[1], bbox[2], bbox[3]), outline=(0, 255, 0))
            draw.rectangle((bbox[0]-0.5, bbox[1]-0.5, bbox[2]+0.5, bbox[3]+0.5), outline=(0, 255, 0))
            draw.rectangle((bbox[0]-1, bbox[1]-1, bbox[2]+1, bbox[3]+1), outline=(0, 255, 0))
            draw.rectangle((bbox[0]-1.5, bbox[1]-1.5, bbox[2]+1.5, bbox[3]+1.5), outline=(0, 255, 0))
            # cv2.rectangle(temp, left_top_point, right_down_point, (0, 255, 0), 4)
            sname = image_dir + '%d_%d_%s' % (i, j, word) + '.jpg'
            # cv2.imwrite(sname, temp)
            temp_img.save(sname, 'jpeg')
